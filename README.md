...
成本统计不使用日志中的 `quota` 字段。按 token 计费时，PClaw 使用输入 token、输出 token、模型倍率、输出倍率与分组倍率计算美元基础成本；按次计费时使用模型固定美元价格与分组倍率。两种结果都会乘以 New API 当前人民币汇率和 `1.1`，最终以人民币显示。无法匹配模型价格、分组倍率或汇率的调用不会被静默估算。
