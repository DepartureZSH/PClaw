...
- 按 New API 模型价格与当前令牌消费日志计算本周美元成本，支持 7 天统计图；按 token 与按次模型均统一加 10%
...
成本统计不使用日志中的 `quota` 字段。按 token 计费时，PClaw 使用输入 token、输出 token、模型倍率、输出倍率与分组倍率换算成本；按次计费时使用模型固定价格与分组倍率，两种结果均乘以 `1.1`。无法匹配模型价格或分组倍率的调用会标记为“未计价”，不会被静默估算。
